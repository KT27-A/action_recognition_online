...
def download_clip(video_identifier, output_filename,
    ...
    print('downloading {}'.format(video_identifier))
    tmp_filename = os.path.join(tmp_dir,
                                '%s.%%(ext)s' % uuid.uuid4())
    command = ['youtube-dl', '--cookies=newcookiefile.txt',
               '--quiet', '--no-warnings',
               '-f', 'mp4',
               '-o', '"%s"' % tmp_filename,
...
def download_clip_wrapper(row, trim_format, tmp_dir):
    """Wrapper for parallel processing purposes."""
    output_filename = row.strip('\n').split('/')[-1]
    st, et = output_filename[12:-4].split('_')
    clip_id = output_filename[:11]

    downloaded, log = download_clip(clip_id, output_filename,
                                    int(st), int(et),
                                    tmp_dir=tmp_dir)
...
def main(input_file, output_dir,
    ...
    if num_jobs == 1:
        status_lst = []
        for row in data:
            status_lst.append(download_clip_wrapper(row,
                                                    trim_format, tmp_dir))
    else:
        status_lst = Parallel(n_jobs=num_jobs)(delayed(download_clip_wrapper)(
            row,
            trim_format, tmp_dir) for row in data)
...
if __name__ == '__main__':
    ...
    p.add_argument('--input_file', type=str, 
                    default='./not_open_file.txt', 
                   help=('file containing the following format: '
                         'YouTube Identifier,Start time,End time,Class label'))
    p.add_argument('--output_dir', type=str, 
                    default='video path /media/alien/Training/Kinetics/kinetics/kinetics_400_test/',
                   help='Output directory where videos will be saved.')
    p.add_argument('-f', '--trim-format', type=str, default='%06d',
                   help=('This will be the format for the '
                         'filename of trimmed videos: '
                         'videoid_%0xd(start_time)_%0xd(end_time).mp4'))
    p.add_argument('-n', '--num-jobs', type=int, default=6) # default 24
